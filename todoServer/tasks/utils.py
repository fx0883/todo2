...
def generate_repeat_dates(
    ...
    Returns:
        重复日期列表
    """
    print(f"Generating dates with: type={repeat_type}, config={repeat_config}, start={start_date}, end={end_date}")
    ...
    if repeat_type == 'custom':
        custom_days = repeat_config.get('custom_days', [])
        print(f"Custom repeat with days: {custom_days}")
        ...
        while True:
            ...
            print(f"Checking date: {current_date}, weekday: {weekday}")
        ...
        while True:
            ...
            if repeat_type == 'daily':
                ...
                if current_date.weekday() == 0:  # 周一
                    dates.append(current_date)
                    current_date += timedelta(days=7 * interval)
                else:
                    current_date += timedelta(days=1)
    ...
    print(f"Generated {len(dates)} dates: {dates}")
    return dates
